...
View.prototype.initSequence = function(target){
	...
	if(seq.attr('id')!=='unusedNodes'){
		...
		toggleLink = '';
		if(isActive){
			toggleLink = $('<a class="tooltip" title="' + view.getI18NString('hide') + '"><img class="icon" src="../../images/icons/svg/dark-gray/hide.svg"/ alt="hide"></a>');
			...
			toggleLink = $('<a class="tooltip" title="' + view.getI18NString('show') + '"><img class="icon" src="../../images/icons/svg/dark-gray/show.svg"/ alt="show"></a>');
		...
		var copyLink = $('<a class="tooltip" title="' + view.getI18NString('duplicate') + '"><img class="icon" src="../../images/icons/svg/dark-gray/duplicate.svg" alt="duplicate"/></a>');
		var deleteLink = $('<a class="tooltip" title="' + view.getI18NString('delete') + '"><img class="icon" src="../../images/icons/svg/dark-gray/trash.svg" alt="delete"/></a>');
...
View.prototype.editSequence = function(target){
	...
	function populateSequence(resetDisplay) {
		...
		$('li.node',content).each(function(){
			...
			editButton.html('<img class="icon" alt="edit" src="../../images/icons/teal/edit.png"></img>' + view.getI18NString('authoring_project_content_edit_step'));
...
View.prototype.retrieveProjectInfoSuccess = function(text,xml,o) {
	...
	if(projectOwnerUsername !== view.portalUsername){
		...
		$('#topProjectTools').before('<img id="sharedIcon" class="infoIcon" alt="shared" src="../../images/icons/red/multi-agents.png" title="' + sharedTitle + '" />');
	}

	// if project is a library project, add library icon
	if(isLibrary){
		var libraryTitle = view.getI18NString("authoring_project_library");
		$('#topProjectTools').before('<img id="libraryIcon" class="infoIcon" alt="library project" src="../../images/icons/red/bookmark.png" title="' + libraryTitle + '" />');
	}
...
View.prototype.populatePortalProjects = function(t,copyMode){
	...
	for(var x=projectsArray.length-1; x>-1; x--) {
		...
		var isOwned = false, iconSrc = '../../images/icons/red/shared.png', iconAlt = openDialogSharedText;
		var sharedDetails = '', sharedUsers = '';
		if(projectOwner === this.portalUserFullname){
			isOwned = true;
			iconSrc = '../../images/icons/teal/briefcase.png';
			iconAlt = openDialogOwnedText;
		} else {
			if(isAuthorable){
				sharedDetails = '<p class="sharedDetails">' +
				'<img alt="shared" src="../../images/icons/red/shared.png" />' +
		...
		if(typeof project.isLibrary === 'boolean' && project.isLibrary){
			...
			infoIcon = '<img alt="favorite" src="../../images/icons/brown/bookmark.png" title="' + infoTitle + '">';
			libraryDetails = '<p class="libraryDetails">' +
			'<img alt="favorite" src="../../images/icons/red/bookmark.png" />' +
			infoTitle +
			'</p>';
			...
			infoIcon = '<img alt="copy" src="../../images/icons/brown/copy-item.png" title="' + infoTitle + '">';
		...
		'</div>' +
		'<div class="' + createdClass + 'projectDetails">' + createdPre + dateCreated + '<img class="info" alt="more info" src="../../images/icons/svg/teal/info.svg" data-projectid="' + projectId + '" data-tooltip-class="info" data-tooltip-anchor="left" data-tooltip-event="click" data-tooltip-keep="true"/></div>' +
		'</div>' +
		'</div>';
